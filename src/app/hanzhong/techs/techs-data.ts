import { mapTableToResourcesAndBonuses } from '../data';
import type { HanzhongTechType } from './TechType';

const assetPath = (f: string): string => `hanzhong/tech/${f}.png`;

const HANZHONG_TECH = `hanzhong--tech`;
export const HANZHONG_TECH_IDS: Record<string, string> = {
  //  ----- War Tier 1 -----
...
const __CACHED_TECHS_BY_NAME: __CacheRecord = __TECHS.reduce<__CacheRecord>(
  (cache: __CacheRecord, tech: HanzhongTechType) =>
    ({
      ...cache,
      [tech.label]: tech,
...
export const getTechIdByName = (name: string): string => __CACHED_TECHS_BY_NAME[name].id;
