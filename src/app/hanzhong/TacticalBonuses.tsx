...
export const TacticalBonuses = () => {
  ...
  const [items, setItems] = useState<HanzhongInfoDataType[]>(calculateSpecialTrainingsTacticalPoints(getValue, bonuses));

  const configure = () => navigate(HANZHONG_ROUTES.FORMATIONS);

  useEffect(() => {
    setItems(calculateSpecialTrainingsTacticalPoints(getValue, bonuses));
  }, [bonuses]);
